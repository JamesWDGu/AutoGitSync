...
项目自身的代码、文档和 agent 资源采用 [0BSD](LICENSE)。PR 和 star 是邀请，不是许可条件。
请只贡献你有权按此协议提供的内容。Python、Git、基础镜像等第三方组件保留各自的许可证和声明。

## Agent 开发说明

[AGENTS.md](AGENTS.md) 保存可公开的通用开发约定，统一使用英文。
本机路径、工具可用性和非敏感偏好放在可选的根目录 `AGENTS.local.md` 中，同样使用英文。
该文件及常见编辑器/备份副本被 Git 忽略，并排除在 Docker 构建上下文之外。
新克隆和 CI 不需要它；agent 仅在相关任务需要时读取，不得把内容复制到公开产物。

两个文件都不要存放凭据。ignore 不会阻止本地工具或 agent 读取文件，也不会取消已有跟踪或清除
Git 历史。提交前检查暂存差异；文档链接只是读取提示，不是自动导入机制。
这次拆分不会新增应用配置文件。
...
对 `main` 或 `release` 的普通推送、PR 和手动运行工作流都只验证，不发布镜像。
...
发布必须同时依赖**单元测试、容器冒烟测试和发布校验关卡**。
日常开发在 `main` 进行。只有面向用户的修复或功能值得发版并获得授权时，才把选定代码推进到
长期保留的 `release` 分支，设置源码版本，等待该分支最新提交的两个工作流全部通过，再在这个
已验证提交上推送带说明的 `vMAJOR.MINOR.PATCH` 标签。校验关卡会拒绝：缺失 `release` 分支、
标签不指向远端 `release` 最新提交、轻量/预发布/非规范标签，以及源码版本与标签不一致。
发布及核验完成前，不要继续推进 `release`。

只有上述标签推送才发布版本/minor、`:latest` 和 SHA 镜像标签，并创建 GitHub Release。
不提供手动强制发布开关，也不更新 `:main` 或 `:release` 镜像。需要可复现部署时请固定版本标签。
仅文档/CI 的修改留在 `main`，等有意义的版本再一起发布，不为保持两个分支 SHA 一致而发版。

首次采用此策略发版时，获得发布授权后，从包含新工作流的已审查 `main` 提交创建 `release`。
后续通过正常快进或经过审查的合并推进，禁止强推。发版后把仅在 `release` 上产生的版本更新及
修复合并回 `main`，确保下次推进时保留。已有镜像和历史标签不动：`:main` 停止更新，现有
`:latest` 仅在下一次获批的稳定版发布时替换。使用 `:main` 的用户应主动切换到稳定版本或
`:latest`，此策略不会自动迁移任何部署。
